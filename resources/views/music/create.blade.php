...
<nav aria-label="breadcrumb" class='mb-5'>
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">音楽のアップロード</li>
  </ol>
</nav>

<form class='card form' method="POST" action="{{route('music.store')}}" enctype="multipart/form-data">
  @csrf
  <div class="card-header"></div>
  <div class="card-body overflow-hidden">
    <div class="form-group">
      <label class='form__title color-main bold' for="title">フレーズのタイトル</label>
      <input type="text" class="form-control" id="title" name='music[title]' value="{{ old('music.title') }}" placeholder="フレーズのタイトル">
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <textarea class="form-control" name="music[description]" id="description" rows="5" placeholder="フレーズの説明">{{ old('music.description') }}</textarea>
    </div>
    ...
    <div class='row mb-3'>
      ...
      <div class='col-md-12 col-lg-8'>
        <img id='music_image' class="music__image" src="" >
      </div>
    </div>
    <button type='submit' class='btn btn-primary btn-lg'>フレーズを投稿</button>
  </div> <!-- card -->
</form>
@endsection